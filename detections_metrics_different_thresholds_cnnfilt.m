...
for isess = 1:length(dirSessions)
    ...
    thrs = {};
    thrs.filt = 2:8;
    thrs.cnn = 0.2:0.1:0.8;
    for ithr = 1:7
        fprintf('thresh %.1f...', ithr);

        % CNN
        file_name = fullfile(dirData, 'events', ['events_cnn32_thr', num2str(thrs.cnn(ithr))]);
        pred_events = read_all_events([file_name '.txt'], 'exact_match', 1);
        cnn_events = pred_events(1,:);
        for ii = 2:size(pred_events,1)
            if all(pred_events(ii,:) < cnn_events(end,2)+0.032)
                cnn_events(end, 2) = pred_events(ii,2);
            else
                cnn_events = [cnn_events; pred_events(ii,:)];
            end
        end
        
        % Filter        
        file_name = fullfile(dirData, 'events', ['events_butter_thr', num2str(thrs.filt(ithr)) ]);
        filt_events = read_all_events(file_name, 'exact_match', 1);

        
        

        % Check which ones are TP or FP
        [precision, recall, F1, TP, FN, IOU] = compute_precision_recall_events({cnn_events}, true_events);

        % Compute properties
        properties.detection = compute_ripple_properties(LFP, cnn_events, ch.pyr(true_shank), fs);
        ...
        save([file_name '_metrics.mat'], 'metrics', 'properties')
